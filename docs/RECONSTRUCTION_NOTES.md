...
An `npm audit --omit=dev` check on 2026-08-09 reports moderate/high advisories in the Expo/React Native/Metro build dependency tree. npm's proposed force fix upgrades Expo back to SDK 57, which would break physical-iPhone Expo Go compatibility and was not applied. There are no reported critical advisories and no hand-written use of the affected parsers/build paths. Recheck Expo's upstream releases before distributing beyond the internal prototype; do not run `npm audit fix --force` without reviewing the SDK compatibility change.
